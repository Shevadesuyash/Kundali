...
def test_initial_dasha_lord_mapping():
    # Ashwini (0) -> Ketu (7)
    dashas = VimshottariCalculator.calculate_dasha_periods("2000-01-01T00:00:00+00:00", 0.0, 0)
    assert dashas[0]["planet"] == "Ketu"
    assert dashas[0]["dasha_years"] == 7.0
    
    # Bharani (1) -> Venus (20)
    dashas = VimshottariCalculator.calculate_dasha_periods("2000-01-01T00:00:00+00:00", 13.333333333333334, 1)
    assert dashas[0]["planet"] == "Venus"
    assert dashas[0]["dasha_years"] == 20.0
    
    # Revati (26) -> Mercury (17)
    dashas = VimshottariCalculator.calculate_dasha_periods("2000-01-01T00:00:00+00:00", 346.6666666666667, 26)
    assert dashas[0]["planet"] == "Mercury"
    assert abs(dashas[0]["dasha_years"] - 17.0) < 0.001
...
def test_fractional_balance_calculation():
    ...
    dashas = VimshottariCalculator.calculate_dasha_periods("2000-01-01T00:00:00+00:00", 6.666666666666667, 0)
    assert dashas[0]["planet"] == "Ketu"
    assert abs(dashas[0]["dasha_years"] - 3.5) < 0.001
    ...
    dashas = VimshottariCalculator.calculate_dasha_periods("2000-01-01T00:00:00+00:00", 13.333333333333332, 0)
    assert dashas[0]["planet"] == "Ketu"
    assert dashas[0]["dasha_years"] < 0.001

def test_date_projection_and_total_duration():
    # Start at year 2000
    dashas = VimshottariCalculator.calculate_dasha_periods("2000-01-01T12:00:00+00:00", 0.0, 0)
    
    assert len(dashas) == 9
    
    # Total years should sum exactly to 120
    total_years = sum(d["dasha_years"] for d in dashas)
